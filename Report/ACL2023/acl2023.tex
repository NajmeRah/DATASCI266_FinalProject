...
\texttt{\{shubhambb, ben.robbins, nrahimi\}@ischool.berkeley.edu}
}
...
\cite{perenualAPI}, from which we extracted 3{,}000 plant entries. While this corpus is relatively small for modern NLP tasks, we found no larger publicly available dataset that provides both reliable species labels and sufficiently detailed textual descriptions. As a result, our experiments reflect the constraints of working with a limited but high-quality dataset. Each entry includes structured horticultural attributes (e.g., sunlight requirements,watering needs, soil type, growth cycle) as well as a free-text description of the species. The raw dataset contained 1{,}548 unique common names and 3{,}000 unique scientific names.
...
RecoBERT \cite{malkiel2020recobert} introduces domain-adapted encoders for catalog item retrieval
...
descriptions (with some masked tokens) through \texttt{BertModel}, average the title
...
where instead of using the embedding averages, we took the \texttt{pooler\_output} parts of
the \texttt{BertModel} title and description outputs and computed their cosine similarities.
...
nuances. Specifically, it projects \texttt{BertModel} embeddings onto the dimension of vocabulary
size and uses a loss function that optimizes for a high embedding value for the index
corresponding to the original token.

For evaluation, we ranked candidate recommendations by the sum of (1) the cosine similarity between the user query description and the candidate plant name, and (2) the cosine similarity between the user query description and the candidate plant description, where all embeddings used for cosine similarity are produced by the model.
...
In an attempt to improve the accuracy the same model was then finetuned. For this all but the last layer of the model had their parameters frozen. The Perenual dataset text descriptions with the description generated from ChatGPT based on the Perenual dataset were paired with a cosine similarity of 1. To generate some negative examples the descriptions were shifted 5 places. These were given a cosine similarity of 0. The model was then finetuned over one epoch using the CoSENTLoss loss function \cite{huggingface2023sberttraining} with a batch size of 16 and a warm up ratio of 0.1.
Code: \href{https://github.com/NajmeRah/DATASCI266_FinalProject/blob/main/Code/SBert.ipynb}{SBERT Code}

\section{Results}
For retrieval-style plant identification, the goal is to surface the correct species among a ranked list of candidates, rather than classify every description into a single label. For this reason, to evaluate the various approaches, we decided to use metrics such as \textbf{Top-1}, \textbf{Top-5}, and \textbf{Top-10} accuracy, as they directly measure the quality of ranked recommendations. Traditional metrics like precision and recall assume binary classification and do not reflect how well a model orders candidates or whether the correct plant appears among the top suggestions a user would actually see.
...
\begin{tabular}{lccc}
\hline
\textbf{Model 1} & \textbf{Top-1} & \textbf{Top-5} & \textbf{Top-10} \\
\hline
...
\begin{tabular}{lccc}
\hline
\textbf{Model 2} & \textbf{Top-1} & \textbf{Top-5} & \textbf{Top-10} \\
\hline
...
\noindent\textbf{Table 2:} Retrieval accuracy on the Wikipedia evaluation set using the
rule-based flattened \texttt{text} representation of the Perenual data.


\subsection{Comparison of Results}

Across both versions of the Perenual dataset discussed at the beginning of the
\textit{Models} section, Advanced Sentence-BERT consistently outperforms the
TF--IDF baseline and RecoBERT. TF--IDF, while simple and interpretable, shows
limited retrieval capacity, although the Top-1 accuracy increases from 0.3\% for
the generated-descriptions variant to 3.3\% for the rule-based variant.
RecoBERT performs better than TF--IDF at higher $K$ values, suggesting that its
learned alignment between plant names and descriptions provides some semantic
signal; however, its Top-1 accuracy remains low, indicating difficulty in
selecting the single correct species. Advanced Sentence-BERT shows the strongest
performance across all metrics, achieving the highest Top-1, Top-5, and Top-10
accuracies for both Perenual dataset variants. This demonstrates the ability of
its sentence encoder to capture richer semantic representations of plant
descriptions.

\section*{Conclusion}
While we were able to improve upon the baseline, our struggle to improve it to a point of getting close to half of the results in the top 10 predicted shows the difficulty of the task. This first and foremost appears to us to be a victim of not enough differentiating information between classes. While we tried to combat a lack of relatively low number of examples per plant species by rewriting the examples with AI, the large number of descriptions combined with the similarity of species made it difficult to differentiate without more information. For example, the SBERT model would predict fig instead of common fig, tall fescue instead of fescue, sweet fennel instead of fennel, and often mix up the many species of apple trees that were in the list. Additionally, having a limited number of descriptions for each plant made it harder for the models to learn unique traits for each of them.

Another thing that may be hindering our results is the prompt phrasings of the generated sets of data. Specifically, generated user versions of plant queries include filler words that distract from the core content to be compared. In our process to build the models we also would occasionally run the models on the raw wikipedia descriptions instead of the generated user versions. These yielded much better results though we chose to exclude them from our results as we thought they were less authentic to what a human user would query. Given more time and data these models could drastically improve off of the final state of the project.

\section*{Authors' Contributions}

All team members contributed to dataset exploration, data cleaning, and the construction of both text-based representations (rule-based and LLM-generated). All authors participated in model evaluation, error analysis, and writing the final report.
\textbf{Najmeh Rahimi} implemented the TF--IDF baseline model.
\textbf{Ben Robbins} implemented the advanced Sentence-BERT model.
\textbf{Shubham Banavalikar} implemented the RecoBERT model.
